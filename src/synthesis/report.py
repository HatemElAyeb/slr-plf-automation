...
SECTION_GUIDANCE = {
    "Introduction": (
        "Provide context about PLF, motivate the research question, and outline what "
        "this review covers (mention the corpus size from the statistics). "
        "2-3 paragraphs."
    ),
    "Results": (
...
def generate_report(question_id: str, question_text: str, output_path: str | None = None) -> str:
    ...
    md.append("\n## 2. Methodology\n")
    md.append(
        "This review was generated by an automated SLR pipeline that:\n"
        "1. Built boolean search queries from the research question via LLM (Module 0)\n"
        "2. Collected papers from PubMed, OpenAlex, ArXiv, MDPI, and Springer\n"
        "3. Embedded title+abstract and indexed them in a Qdrant vector database\n"
        "4. Screened each paper using LLM-applied inclusion/exclusion criteria\n"
        "5. Extracted structured data from included papers (full text via RAG, "
        "abstract fallback when PDF unavailable)\n"
        "6. Synthesized this report from the extracted data\n"
    )
